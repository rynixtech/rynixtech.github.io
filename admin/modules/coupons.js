            ...
            return `
                <div style="background: #0f1425; border-radius: 8px; border: 1px solid rgba(183,202,255,0.12); padding: 20px;">
                    ...
                    <div style="display: flex; gap: 8px;">
                        <button onclick="window.edit_coupons('${escapeHTML(doc.id)}')" style="flex: 1; background: rgba(85,220,255,0.1); border: 1px solid rgba(85,220,255,0.2); color: #55dcff; padding: 8px; border-radius: 4px; cursor: pointer;">Edit</button>
                        <button onclick="window.delete_coupons('${escapeHTML(doc.id)}')" style="background: rgba(255,117,143,0.1); border: 1px solid rgba(255,117,143,0.2); color: #ff758f; padding: 8px; border-radius: 4px; cursor: pointer;">Delete</button>
                    </div>
                </div>
            `;
        }).join('');
...
window.openModal_coupons = (docId = '', title = '', details = '') => {
    const content = document.createElement('div');
    content.innerHTML = `
        <form id="coupons-form" style="display: flex; flex-direction: column; gap: 16px;">
            <input type="hidden" id="coupons-doc-id" value="${escapeHTML(docId)}">
            <div>
                <label style="display:block; margin-bottom:4px; color:#aeb8d2;">Name / Title</label>
                <input type="text" id="coupons-title" required value="${escapeHTML(title)}" style="width: 100%; padding: 8px; background: #0a0e1a; border: 1px solid rgba(183,202,255,0.12); color: white; border-radius: 4px;">
            </div>
            <div>
                <label style="display:block; margin-bottom:4px; color:#aeb8d2;">Details</label>
                <textarea id="coupons-details" rows="3" style="width: 100%; padding: 8px; background: #0a0e1a; border: 1px solid rgba(183,202,255,0.12); color: white; border-radius: 4px;">${escapeHTML(details)}</textarea>
            </div>
    ...
    content.querySelector('#coupons-form').addEventListener('submit', async (e) => {
        e.preventDefault();
        const t = document.getElementById('coupons-title').value;
        const d = document.getElementById('coupons-details').value;
        const did = document.getElementById('coupons-doc-id').value;
        ...
        try {
            if(did) {
                await updateDoc(doc(db, 'coupons', did), data);
            } else {
...
window.edit_coupons = async (docId) => {
    try {
        ...
        if (docSnap.exists()) {
            ...
            window.openModal_coupons(docId, data.title || data.name || '', data.details || data.description || '');
        } else {
            alert('Not found');
        }
    } catch(e) {
