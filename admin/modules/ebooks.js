...
export async function render(container) {
    container.innerHTML = `
        <div class="module-container" style="padding: 20px;">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
                <h2>eBooks</h2>
                <button onclick="document.getElementById('ebooks-modal').style.display='block'" style="background: #55dcff; color: #0a0e1a; border: none; padding: 8px 16px; border-radius: 4px; cursor: pointer;">+ Add New</button>
            </div>
            ...
            <div id="ebooks-modal" style="display: none; position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(10,14,26,0.9); z-index: 1000; padding: 20px; overflow-y: auto;">
                <div style="background: #0f1425; border: 1px solid rgba(183,202,255,0.12); padding: 24px; border-radius: 8px; max-width: 500px; margin: 40px auto; position: relative;">
                    <button onclick="document.getElementById('ebooks-modal').style.display='none'" style="position: absolute; top: 16px; right: 16px; background: none; border: none; color: #aeb8d2; font-size: 20px; cursor: pointer;">×</button>
                    <h3>Add/Edit Entry</h3>
                    <form id="ebooks-form" style="display: flex; flex-direction: column; gap: 16px;">
                        <input type="hidden" id="ebooks-doc-id">
                        <div>
                            <label style="display:block; margin-bottom:4px; color:#aeb8d2;">Name / Title</label>
                            <input type="text" id="ebooks-title" required style="width: 100%; padding: 8px; background: #0a0e1a; border: 1px solid rgba(183,202,255,0.12); color: white; border-radius: 4px;">
                        </div>
                        <div>
                            <label style="display:block; margin-bottom:4px; color:#aeb8d2;">Details</label>
                            <textarea id="ebooks-details" rows="3" style="width: 100%; padding: 8px; background: #0a0e1a; border: 1px solid rgba(183,202,255,0.12); color: white; border-radius: 4px;"></textarea>
                        </div>
    ...
    document.getElementById('ebooks-form').addEventListener('submit', async (e) => {
        e.preventDefault();
        await saveItem();
    });
...
window.edit_ebooks = async (docId) => {
    try {
        ...
        if (docSnap.exists()) {
            ...
            document.getElementById('ebooks-doc-id').value = docId;
            document.getElementById('ebooks-title').value = data.title || '';
            document.getElementById('ebooks-details').value = data.details || '';
            document.getElementById('ebooks-modal').style.display = 'block';
        } else {
            alert('Not found');
        }
    } catch(e) {
...
async function saveItem() {
    ...
    try {
        ...
        document.getElementById('ebooks-form').reset();
        document.getElementById('ebooks-doc-id').value = '';
        loadItems();
    } catch(e) {
        console.error(e);
