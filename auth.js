...
export async function redirectBasedOnRole(user) {
    if (!user) return;
    const returnTo = sessionStorage.getItem('rynix_return_to');
    if (returnTo && /^(shopping|apps|books|os-store)\.html$/.test(returnTo)) {
        sessionStorage.removeItem('rynix_return_to');
